...
export let mousemove = (e, context, ref) => {
  ...
  if (penmovebool.current[0]) {
    console.log(vectorCi.current, "ii");
    ...
    console.log(vectorGp.current, "ahan");
    console.log(vectorCi.current, "if");
    console.log(vectorCi.current[vectorCi.current.length - 2], "ind");

    vDragpointsArr.current = [
      vectorCp.current[vectorCp.current.length - 2],
      vectorCp.current[vectorCp.current.length - 1],
    ];
    if (vDragpointsArr.current.length !== 0) {
      gensyncDragpoint(context);
    }
    ...
    let ctrpoint = vectorCi.current[vectorCp.current.length - 2] + 2;
    ...
    draga.current = [];
    getPathData(context);
    dragx.current = +movepen.current.split(" ")[0];
    dragy.current = +movepen.current.split(" ")[1];
    vectordrag.current = true;
    setPathDataV(
      context,
      +movepen.current.split(" ")[0],
