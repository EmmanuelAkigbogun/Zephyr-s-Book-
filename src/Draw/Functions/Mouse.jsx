  ...
  if (
    ...
    if (vgpathxy.current[`pathmouse`] === undefined && !vectoredit.current) {
      ...
      if (pen.current) {
        ...
        else{downconst.current.push(`L${x} ${y}`);}
          if (
...
export let mousemove = (e, context, ref) => {
      ...
      */
      ...
      if (pen) {
        ...
        if (penmovebool.current[1]) {
          pencirctr.current.splice((pencirctr.current.length)-1,1,movepen.current);
        }
        if (penpause.current) {
        } else {
          if (vectoredit.current) {
            if (vectoredit.current) {
              vgpath.current.splice(
                ...
                1,
                downconst.current.join("") +
                  (penmovebool.current[1]&&1==2
                    ? `C${x} ${y} ${x} ${y} ${x} ${y}`
                    : (penmovebool.current[2]&&1==2
                    ? `C${
                        pencirctr.current[(pencirctr.current.length) - 1]
                      }  ${x} ${y} ${x} ${y}`
                    : `L${x} ${y}`))
  ...
  if (vectoredit.current&&!penpause.current) {
