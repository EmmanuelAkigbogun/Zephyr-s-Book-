  ...
  for (let index = 0; index < e.targetTouches.length; index++) {
    ...
    if (vgpathxy.current[`path${element.identifier}`] === undefined) {
      ...
      //vgcolor.current.push(`hsl(${(element.identifier+1)*20},100%,50%)`);
       //straightcolor.current.push(`hsl(${(element.identifier + 1) * 20},100%,50%)`);
      //      `rgb(${Math.random() * 255},${Math.random() * 255},${
       //   Math.random() * 255
     //   })`
      vgidentity.current[`${element.identifier}`] = vgpath.current.length - 1;
    }
  }
};
export let touchmove = (
  ...
  for (let index = 0; index < e.targetTouches.length; index++) {
    ...
    if (vgpathxy.current[`path${element.identifier}`] !== undefined) {
      ...
      if (
        line &&
        (Math.round(begin.x) === Math.round(end.x) ||
          Math.round(begin.y) === Math.round(end.y))
      ) {
        /*        
        ...
        /*
            ...
            if (
              !Object.values(
                elArr[vgidentity.current[`${element.identifier}`]].attributes
              )
                .map((m) => m.name)
                .includes("stroke")
            ) {
              elArr[vgidentity.current[`${element.identifier}`]]?.setAttribute(
                "stroke",
            ...
            if (
                         !Object.values(
                           elArr[vgpath.current.length - 1].attributes
                         )
                           .map((m) => m.name)
